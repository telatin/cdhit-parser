...
def has_cdhit():
    """
    Checks if cd-hit is installed.
    Returns:
      bool: True if cd-hit is installed, False otherwise.
    """
    cmd = ["cd-hit", "-h"]
    # Run cmd and save stdout as "cdout" variable, ignore exit status
    try:
        cdout = subprocess.run(
...
def read_fasta(path):
    ...
    Yields:
      tuple: A tuple containing the name, comment, and sequence of each entry in the FASTA file.
    Examples:
      >>> for name, comment, seq in read_fasta('example.fasta'):
...
def relabel_fasta(path, prefix, outpath):
    """
    Relabels a FASTA file.
    Args:
      path (str): Path to the FASTA file.
      prefix (str): Prefix to add to the name of each entry.
      outpath (str): Path to the output FASTA file.
    Side Effects:
      Writes the relabeled FASTA file to outpath.
    """
    with xopen(outpath, "a") as out:
        for name, comment, seq in read_fasta(path):
            out.write(
                ">{}{}{}{}\n{}\n".format(
                    prefix, name, " " if len(comment) > 1 else "", comment, seq
                )
            )
...
def split_cluster(cluster, tag1, tag2):
    """
    Splits a cluster into two pools.
    Args:
      cluster (Cluster): The cluster to split.
      tag1 (str): The tag to identify sequences for the first pool.
      tag2 (str): The tag to identify sequences for the second pool.
    Returns:
      tuple: A tuple containing the two pools.
    Raises:
      ValueError: If a sequence does not start with either tag.
    """
    pool1, pool2 = [], []
    for sequence in cluster.sequences:
        if sequence.name.startswith(tag1):
            ...
            raise ValueError(
                "Sequence {} does not start with {} or {}".format(
                    sequence.name, tag1, tag2
                )
            )
...
@click.option(
    "--id", help="Identity threshold [default: 0.9]", default=0.95, type=float
)
...
def compare(
    fasta1, fasta2, tag1: str, tag2: str, tempdir, type: str, id: float, verbose: bool
):
    """
    Compare FASTA files
    ...
    if type is None:
        ...
        print(
            "Type of sequences not specified, assuming {}".format(type), file=sys.stderr
        )

    if prefix1 == prefix2:
        print(
            "Warning: prefixes are identical ({}): specify manual prefixes with --tag1 and --tag2".format(
                prefix1
            )
        )
        exit(1)
    ...
    tags = {TAG1: prefix1, TAG2: prefix2}
    if verbose:
        print(
            "Relabeling {} to {} (prefix: {})".format(fasta1, fasta_file, prefix1),
            file=sys.stderr,
        )
        print(
            "Relabeling {} to {} (prefix: {})".format(fasta2, fasta_file, prefix2),
            file=sys.stderr,
        )

    cmd = [
        "cd-hit" if type == "prot" else "cd-hit-est",
        "-i",
        fasta_file,
        "-o",
        clstr_file,
    ...
    for cluster in read_cdhit(clstr_file + ".clstr"):
        ...
        if pool != TAG1 and pool != TAG2:
            raise ValueError(
                "Cluster {} does not start with {} or {}".format(
                    cluster.refname, TAG1, TAG2
                )
            )
    ...
    for key, list in stats.items():
        ...
        for seqnames in list:
            ...
            key = (
                "dupl_" + tags[key[-1 * len(TAG1) :]]
                if key[-1 * len(TAG1) :] in tags
                else key
            )
            seqnames = seqnames.replace(TAG1, tags[TAG1] + "#").replace(
                TAG2, tags[TAG2] + "#"
            )
